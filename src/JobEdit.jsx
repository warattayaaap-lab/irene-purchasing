      ...
      <div className="card-box">
        ...
        <div className="items-table">
          <div className="it-head">
            ...
            <span className="col-name">ชื่อของ</span>
            <span className="col-qty">จำนวน</span>
            <span className="col-unit">หน่วย</span>
            <span className="col-cmp">เทียบ</span>
            <span className="col-shop">ร้าน</span>
            <span className="col-price">ราคา</span>
            <span className="col-self">ซื้อเอง</span>
            <span className="col-ship">วันส่ง/รับ</span>
            <span className="col-del"></span>
          </div>
          {items.map((it, idx) => (
            <div className="it-row" key={it._k}>
              <span className="col-num">{idx+1}</span>
              <input className="col-name" value={it.name} onChange={e=>setItem(it._k,'name',e.target.value)} placeholder="ชื่อของ" />
              <input className="col-qty" value={it.qty} onChange={e=>setItem(it._k,'qty',e.target.value)} type="number" placeholder="0" />
              <input className="col-unit" value={it.unit} onChange={e=>setItem(it._k,'unit',e.target.value)} placeholder="หน่วย" />
              <span className="col-cmp"><button className={'i-cmp'+(it.compare?' on':'')} onClick={()=>setItem(it._k,'compare',!it.compare)}>{it.compare?'✓':'เทียบ'}</button></span>
              {it.compare ? (
                <><span className="col-shop i-muted">—</span><span className="col-price i-muted">เทียบ</span></>
              ) : (
                <>
                  <input className="col-shop" value={it.shop} onChange={e=>setItem(it._k,'shop',e.target.value)} placeholder="ร้าน" />
                  {it.self_buy ? <span className="col-price i-muted" style={{color:'var(--danger)'}}>รอบิล</span>
                    : <input className="col-price" value={it.price} onChange={e=>setItem(it._k,'price',e.target.value)} type="number" placeholder="0" />}
                </>
              )}
          ...
          <div style={{padding:'8px 6px 2px'}}>
            <button className="add-row-link" onClick={addItem}>+ เพิ่มรายการ</button>
          </div>
        </div>

        {/* ปุ่มเครื่องมือ + สั่งตรงร้านเดียว */}
        <div className="item-tools">
          <button className="btn ghost" onClick={copyItems}>📋 คัดลอกส่งร้าน</button>
          <button className="btn ghost" onClick={()=>{const s=prompt('สั่งตรงร้านเดียวทั้งงาน — พิมพ์ชื่อร้าน:');if(s){setOneShop(s);setTimeout(applyOneShop,0)}}}>🏪 สั่งตรงร้านเดียวทั้งงาน</button>
          <button className="btn ghost" onClick={()=>setPasteBox(!pasteBox)}>📋 วางรายการจากไลน์</button>
        </div>
        <div style={{display:'none'}}>
          <input value={oneShop} onChange={e=>setOneShop(e.target.value)} />
          <button onClick={applyOneShop}>ใช้ร้านนี้ทุกรายการ</button>
        </div>

        {/* วางรายการจากไลน์ (กล่องพิมพ์) */}
        {pasteBox && (
